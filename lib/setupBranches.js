...
function writeApacheConfig(branch, name, config) {
  ...
  const branchApacheConfigPath = `${apacheConfigDir}/branch_${name}.conf`;
...
module.exports = function setupBranches(branchList, config) {
  return new Promise((resolve, reject) => {
    ...
    Promise.resolve(Object.keys(allBranches))
      .map(branch => writeApacheConfig(allBranches[branch], branch, config), {
        concurrency: 1
      })
      .then(updated =>
        runPostBuildProcess(updated.some(s => s === true), config)
      )
