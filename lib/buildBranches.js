    ...
    return new Promise((resolve, reject) => {
        ...
        try {
            git
                ...
                .clean('f', { '-d': null, '-fx': ""})
                .checkout(name)
                .then(() => logVerbose(`[${owner}/${repo}#${name}]: Clean and checkout base repo`))
                .clean('f', { '-d': null, '-fx': ""})
                .pull()
                ...
                .checkout(`origin/${name}`)
                .then(() => logVerbose(`[${owner}/${repo}#${name}]: git checkout origin/${name}`))
                .pull()
                .then(() => logVerbose(`[${owner}/${repo}#${name}]: git pull`))
                .then(() => resolve());
        } catch(err) {
            log(`[${owner}/${repo}]: ${err}`);
        }
