...
const { lockFileExists, createLockFile, deleteLockFile } = require("./lock");
...
function buildBranch(config, branch, buildPath) {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (!fs.accessSync(`${branchPath}/.git`, fs.constants.F_OK)) {
      log(`[${owner}/${repo}]: ${branchPath} was not a git repo for some reason. Removing and it to try again.`);
      exec(`rm -rf ${branchPath}`);
    }
    if (!fs.accessSync(branchPath, fs.constants.F_OK)) {
      ...
      if (fs.accessSync(`${branchPath}/.build.txt`, fs.constants.F_OK)) {
...
function buildProcess(
  ...
  return Promise.resolve(build)
    .mapSeries(
      ({ proc, args = [], env = {}, proceedOnFailure = false }) => !lockFileExists(name) &&
        runBuildProcess(
          { owner, repo },
  ...
  return new Promise((resolve, reject) => {
    ...
    const child = spawn(
      ...
      args.map(arg => {
        ...
        return fs.accessSync(branchPath, fs.constants.F_OK) ? branchPath : masterPath;
      }),
