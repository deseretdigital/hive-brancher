...
const { getWhitelist, getSubdomainFromBranchName } = require("./../lib/getWhitelist");

function deleteBranch({ branch, repos }, config, whitelist) {
  ...
  return new Promise((resolve, reject) => {
    Promise.resolve(repos)
      ...
      .map(
        repo => {
          const subdomain = getSubdomainFromBranchName(branch, whitelist);
...
module.exports = function cleanupBranches(branchSets = [], config, whitelist) {
  return new Promise((resolve, reject) => {
    ...
    Promise.resolve(flaggedForDeletion)
      .map(obj => deleteBranch(obj, config, whitelist))
